...
do i=1,Nocc
 do j=1,Nocc
  do a=Nocc+1,Mbasis2
   do b=Nocc+1,Mbasis2
    td(i,j,a,b)=spin_int(i,j,a,b,ERImol)/(FockM(i,i)+FockM(j,j)-FockM(a,a)-FockM(b,b))
   enddo
  enddo
 enddo
enddo
...
subroutine ccsd_write_last()
implicit none
! Arguments
! Local variables
integer::i,j,a,b
! Procedures
open(unit=iunit,form='unformatted',file='t_amps')
do i=1,Nocc
 do j=1,Nocc
  do a=Nocc+1,Mbasis2
   do b=Nocc+1,Mbasis2
...
do i=1,Nocc
 do a=Nocc+1,Mbasis2
  ...
  do j=1,Nocc
   do b=Nocc+1,Mbasis2
    ccsd_energy=ccsd_energy+one_fourth*spin_int(i,j,a,b,ERImol)*td(i,j,a,b)+half*spin_int(i,j,a,b,ERImol)*ts(i,a)*ts(j,b)
   enddo
  enddo
 enddo
enddo
...
ccsd_en_nof=ccsd_en_nof+one_fourth*spin_int(i,j,a,b,ERImol)*td(i,j,a,b)+half*spin_int(i,j,a,b,ERImol)*ts(i,a)*ts(j,b)
   enddo
   do j=Ndocc+1,Nsocc
ccsd_en_nof=ccsd_en_nof+half*(one_fourth*spin_int(i,j,a,b,ERImol)*td(i,j,a,b)+half*spin_int(i,j,a,b,ERImol)*ts(i,a)*ts(j,b))
   enddo
  enddo
  do i=Ndocc+1,Nsocc
   do j=1,Ndocc
